...
###talvez encerrar esse script aqui e mudar pra análise
```{r}
library(readxl)
library(tidyverse)
library(metafor)
...
Para análises, precisamos da média e n dos dois grupos e da variacao no tratado, seja SEM, SD ou Unclear.
```{r}
...
dados_meta_smd %>% arrange(-vi) %>% select(c("vi")) %>% slice(1:10)
...
Abeta concentration
```{r}
meta_concent = rma.mv(yi=yi, V=vi, data = dados_meta_smd, method = "REML", random = ~1|rayyan.key/Comparison.ID, mods = ~dados_meta_smd$Concentration_uM)
...
```{r}
ggplot(dados_meta_smd, aes(x = Concentration_uM, y=yi, size = 1/sqrt(vi))) + 
  geom_point(shape = 1, stroke = 1) + 
...
(aqui ainda falta a análise por densidade celular, falta limpar/organizar os dados)
...
**obs! revisar depois para adicionar cell density
