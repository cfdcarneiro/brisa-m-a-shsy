...
ggsave("./Figures/funnel-plot-smd-N.png")
...
fig_agregation_ = 
  ...
  labs(y = "Effect size (Hedge's g)", x = expression("A"*beta ~ "aggregation")) + 
  theme_classic() + 
  scale_size_continuous(guide = "none") + 
  theme(legend.position = "bottom") + 
  scale_y_continuous(n.breaks = 7) + 
...
fig_concent_ = 
  ...
  geom_point(shape = 21, fill = "grey", stroke = 0.5) + 
  labs(y = "Effect size (Hedge's g)", x = expression("A"*beta ~ "concentration ("*mu*"M)")) + 
  theme_classic() + 
  scale_size_continuous(guide = "none") + 
  theme(legend.position = "bottom") + 
  scale_y_continuous(n.breaks = 7) + 
...
fig_duration_ =
  ...
  geom_point(shape = 21, fill = "grey", stroke = 0.5) + 
  labs(y = "Effect size (Hedge's g)", x = expression("A"*beta ~ "duration of exposure (days)")) + 
  theme_classic() + 
  scale_size_continuous(guide = "none") + 
  theme(legend.position = "bottom") + 
  scale_y_continuous(n.breaks = 7) + 
...
```{r}
fig_density_1000 = 
  ggplot(dados_meta_smd, aes(x = as.numeric(cell_density_1000), y=exp(yi), size = 1/sqrt(vi))) + 
  geom_point(shape = 21, fill = "grey", stroke = 1) + 
...
```{r}
dados_meta_smd_max100$Abeta_aggregation = factor(dados_meta_smd_max100$Abeta_aggregation, levels = c("Monomers", "Fibers", "Oligomers", "Unclear"))
                                       
dados_meta_smd_max100$Assay = factor(dados_meta_smd_max100$Assay, levels = c("MTT", "MTS", "WST"))
...
pre-registered variables -- c("Diferentiation_method", "Abeta_aggregation", "Assay", "Diferentiation_duration_days", "Concentration_uM", "Duration_days", "Cell_density")
